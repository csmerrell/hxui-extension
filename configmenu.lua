...
config.DrawWindow = function(us)
    ...
    if(showConfig[1] and imgui.Begin(("HXUI Config"):fmt(addon.version), showConfig, bit.bor(ImGuiWindowFlags_NoSavedSettings))) then
        ...
        if (imgui.CollapsingHeader("Party List")) then
            ...
            if (imgui.Checkbox('Show Treasure Icon', { gConfig.showPartyListTreasureIcon })) then
                gConfig.showPartyListTreasureIcon = not gConfig.showPartyListTreasureIcon;
                UpdateSettings();
            end
            imgui.ShowHelp('Display a treasure icon in the party list when treasure pool items are available');
        ...
        if (imgui.CollapsingHeader("Treasure Pool")) then
            imgui.BeginChild("TreasurePoolSettings", { 0, 200 }, true);
            if (imgui.Checkbox('Enabled', { gConfig.showTreasurePool })) then
                gConfig.showTreasurePool = not gConfig.showTreasurePool;
                UpdateSettings();
            end

            local scaleX = { gConfig.treasurePoolScaleX };
            if (imgui.SliderFloat('Scale X', scaleX, 0.1, 3.0, '%.2f')) then
                gConfig.treasurePoolScaleX = scaleX[1];
                UpdateSettings();
            end

            local fontOffset = { gConfig.treasurePoolFontOffset };
            if (imgui.SliderInt('Font Scale', fontOffset, -5, 10)) then
                gConfig.treasurePoolFontOffset = fontOffset[1];
                UpdateSettings();
            end

            local arrowScale = { gConfig.treasurePoolArrowScale };
            if (imgui.SliderFloat('Arrow Scale', arrowScale, 0.1, 3.0, '%.2f')) then
                gConfig.treasurePoolArrowScale = arrowScale[1];
                UpdateSettings();
            end

            if (imgui.Checkbox('Show Controller Legend', { gConfig.showTreasurePoolControllerLegend })) then
                gConfig.showTreasurePoolControllerLegend = not gConfig.showTreasurePoolControllerLegend;
                UpdateSettings();
            end
            
            if (imgui.Checkbox('Consolidate Items', { gConfig.treasurePoolConsolidateItems })) then
                gConfig.treasurePoolConsolidateItems = not gConfig.treasurePoolConsolidateItems;
                UpdateSettings();
            end
            imgui.ShowHelp('Group duplicate items together with separate rows for different lot states');
